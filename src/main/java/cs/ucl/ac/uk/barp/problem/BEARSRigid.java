 ...
 */
public class BEARSRigid extends AbstractIntegerProblem implements ConstrainedProblem<IntegerSolution> {
	...
	public BEARSRigid(Project project) {
	...
	private void computeFitness(IntegerSolution solution) {
		...
		for (int j = 0; j < ConfigSetting.NUMBER_OF_SIMULATION; j++) {
			...
			for (int i = 1; i <= noOfReleases; i++) {
				...
				if (release != null) {
					effortPerRelease[i-1] += computeEffort(release, j);
		...
		double expLateness = computeLatenessProbability(rPlan);
