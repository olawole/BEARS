...
public class Pareto {
	...
	public void setSolutions(List<IntegerSolution> optimalSol, SRPRiskProject projectId) {
	...
	private void addReleasePlan(IntegerSolution solution, SRPRiskProject projectId) {
		Plan rPlan = new Plan(solution, projectId);
//		String planString = planToString(rPlan);
		if (!solutions.contains(rPlan)){
			rPlan.setExceedProbability(Math.abs(solution.getObjective(0)));
			rPlan.setSatisfaction(Math.abs(solution.getObjective(1)));
			solutions.add(rPlan);
//			uniqueSolutions.add(planString);
	...
	public boolean all(Plan plan1, Plan plan2) {
		boolean value = false;
		if (plan1.getExceedProbability() <= plan2.getExceedProbability()
				&& plan1.getSatisfaction() >= plan2.getSatisfaction()) {
			value = true;
		}
		return value;
	}
