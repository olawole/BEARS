...
public class TableGenerator {
	...
	public void generateLatexTable(){
		...
		heading += "Exceed Probability" + COL_SEP + "Satisfaction";
		latexString += heading + "\\\\ \n";
		latexString += HORIZONTAL_LINE;
		int counter = 0;
		for (Plan plan : rPlans){
			...
			row += StatUtil.round(plan.getExceedProbability(), 4) + COL_SEP + 
					StatUtil.round(plan.getSatisfaction(), 2);
			latexString += row + "\\\\ \n";
			latexString += HORIZONTAL_LINE;
		}
	...
	public void generateCSVTable(){
		...
		heading += "Exceed Probability" + COMMA_SEP + "Satisfaction";
		csvString += heading + "\n";
		int counter = 0;
		for (Plan plan : rPlans){
			...
			row += StatUtil.round(plan.getExceedProbability(), 4) + COMMA_SEP + 
					StatUtil.round(plan.getSatisfaction(), 2);
			csvString += row + "\n";
		}
		try {
